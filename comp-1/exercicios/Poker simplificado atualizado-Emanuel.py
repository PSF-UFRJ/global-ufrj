...
class NumCartasPossibilidadesErrado(Exception): pass

class MaoParcialErrada(Exception): pass

class NumCartasParaFormarMaoErrado(Exception): pass
...
# Questão 1 - c) Função auxiliar
#
def nao_contribui_retorno(mao, cartas_permitidas, a, b):
    menor_elemento = elemento_menos_numeroso(mao)
    cartas_retorno = [menor_elemento]*b
    cartas_retorno.extend(cartas_permitidas*a)
    random.shuffle(cartas_retorno)
    return cartas_retorno
...
def nao_contribui(mao):
    valor = valor_mao(mao)
    todas_cartas = {"A", "K", "Q", "J", "7"}
    cartas_permitidas = list(todas_cartas.difference(set(mao)))
    if valor == 0:
        return []
    if valor == 1 or valor == 3:    
        random.shuffle(cartas_permitidas)
        return cartas_permitidas
    elif valor == 2:
        return nao_contribui_retorno(mao, cartas_permitidas, 2, 1)
    elif valor == 5:
        return nao_contribui_retorno(mao, cartas_permitidas, 3, 1)
    elif valor == 10:
        return nao_contribui_retorno(mao, cartas_permitidas, 4, 3)
    elif valor == 50:
        return nao_contribui_retorno(mao, cartas_permitidas, 5, 0)
    return []
...
def possibilidades(mao_parcial, num_cartas, cartas_restantes):
    tamanho_max_num_cartas = len(cartas_restantes)
    tamanho_mao_parcial = len(mao_parcial)
    if tamanho_mao_parcial == 5:
        yield mao_parcial
    if tamanho_mao_parcial < 2 or tamanho_mao_parcial > 5:
        raise MaoParcialErrada
    if num_cartas > tamanho_max_num_cartas:
        raise NumCartasPossibilidadesErrado
    if num_cartas < 5 - tamanho_mao_parcial:
        raise NumCartasParaFormarMaoErrado
    cartas_restantes_randomizadas = cartas_restantes.copy()
    random.shuffle(cartas_restantes_randomizadas)
    sorteio = cartas_restantes_randomizadas[:num_cartas]
    combinacoes_parciais = itertools.combinations(
        sorteio, 5 - tamanho_mao_parcial)
    for c in combinacoes_parciais:
...
def possibilidades_da_mao(cartas_mao, cartas_a_remover):
    ...
    return possibilidades(
        mao_parcial, 
        # Não é para combinar apenas um sorteio específico, mas com todas as
        # cartas restantes - portanto o número de cartas a serem sorteadas são
        # todas
        len(cartas_restantes), 
        cartas_restantes)
...
# trocar três cartas, mas não sabemos, em qual caso, a probabilidade de melhora represenata
# o maior ganho.
#
...
def trocas_ganancioso(mao, trocas1=None):
    # Implemente aqui
    mao_atual = mao
    mao_atual > elemento_menos_numeroso
    for mao_atual in mao:
        elemento_menos_numeroso.remove(mao)
        if mao_atual < 2:
            return elemento_menos_numeroso
        else:
            return mao[:1].copy()
...
def desistir_cauteloso(mao_atual, mao_original, trocas_minhas, trocas_outro=None, segundo=False):
    # Implemente aqui
    if valor_mao(mao_atual) < trocas_outro:
        valor_mao(mao_atual) == 1
        return 10
    elif valor_mao(mao_atual) == 2:
        return 3
    else: 
        return False
...
def rodada(estado, tipo="completa"):
    ...
    estado["cartas_restantes"] = cartas.copy()
