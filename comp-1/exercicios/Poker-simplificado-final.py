...
def trocas_aleatorio(mao, trocas1=None):
    ...
    o jogo tendo no máximo 3 cartas permitida para a troca, contudo suas chance de desistencia também é de 50%.
         >>> l = trocas_aleatorio(["A","J","K","Q","Q"], 3) ; len(l) >= 0 and len(l) <= 3
         True
         >>> l = trocas_aleatorio(["A","J","K","Q","Q"], 1) ; len(l) >= 0 and len(l) <= 3
         True
    """
    trocar = []
    for c in mao:
...
def trocas_humano(mao, trocas1=None):
    ...
    3 ele deseja trocar,ele digita as cartas e é impresso quais cartas ele digitou , 
    caso passe da quantidade de carta delimitada, o jogo pede pra ele tentar novamente.
    
    Não é possível fazer DocTest dessa função, porque ela tem interação humana.
    """
    mao_ordenada = mao.copy()
    mao_ordenada.sort()
...
def trocas_ganancioso(mao, trocas1=None):
   ...
   Se uma mão não tem nem um par, ele troca as 3 menores.
    >>> l = trocas_ganancioso(["A","J","K","Q","Q"], 1) ; len(l) == 3
    True
   """ 
...
def trocas_burro(mao, trocas1=None):
    """ O jogador Burro é o jogador que intencionalmente quer perder. Ele sempre trocas suas cartas se elas
    estiverem em maior quantidade e, se sua mão valer mais, depois da troca, ele desiste. Entretanto, se sua
    mão valer menos, ele insiste no jogo. Fatidicamente, sempre perde, a não ser em situações muito limítrofes,
    mesmo contra o jogador aleatório.
    """
...
# funções do jogador arrojado
def trocas_arrojado(mao, trocas1=None):
    mao_atual = mao.copy()
    if valor_mao(mao) == 2:
        menores_cartas = []
        for i in range(3):
            menor = elemento_menos_numeroso(mao_atual)
            menores_cartas.append(menor)
            mao_atual = diferenca_de_lista(mao_atual, [menor])
        return menores_cartas
    return nao_contribui_max_3(mao)
    

def desistir_arrojado(mao_atual, mao_original, trocas_minhas, trocas_outro=None, segundo=False):
    valor_atual = valor_mao(mao_atual)
    valor_original = valor_mao(mao_original)
    if valor_atual > 5:
        return False
    if trocas_outro == 0:
        # Assume que o adversário está blefando, porque a quina é uma possibilidade muito remota
        return False
    elif trocas_outro == 1:
...
# Funções do Jogo:
def rodada(estado, tipo="completa", max_rodadas=5):
    ...
    if tipo in ["estrategica", "completa"]:
        ...
        if tipo == "completa":
            ...
            if estado["jogador1"]["desistir"](mao_atual1, mao_original1, cartas_a_trocar1, len(cartas_a_trocar2)):
            ...
            if estado["jogador2"]["desistir"](mao_atual2, mao_original2, cartas_a_trocar2, len(cartas_a_trocar1), True):
...
def jogo(tipo="completa", tipo1="aleatorio", tipo2="aleatorio", max_rodadas=5):
    jogador1 = {}
    jogador2 = {}
    ...
    if tipo1 == "aleatorio":
        ...
        jogador1 = {"nome": "Arrojado 1", "trocas": trocas_arrojado, 
                    "desistir": desistir_arrojado, "mao_atual": [], 
                    "pontos": 0}
    else:
    ...
    if tipo2 == "aleatorio":
        ...
        jogador2 = {"nome": "Arrojado 2", "trocas": trocas_arrojado, 
                    "desistir": desistir_arrojado, "mao_atual": [], 
                    "pontos": 0}
    else:
    ...
    while rodada(estado, tipo, max_rodadas): 
        estado["rodada"] += 1
        print("\n")
...
###################################################################################
# Jogadores disponíveis para serem testados na função round, abaixo:
###################################################################################

random_player = (trocas_aleatorio, desistir_aleatorio)
human_player = (trocas_humano, desistir_humano)
greedy_player = (trocas_ganancioso, desistir_ganancioso)
cautious_player = (trocas_cauteloso, desistir_cauteloso)
devious_player = (trocas_blefe, desistir_blefe)
dumb_player = (trocas_burro, desistir_burro)

# ##################################################################################
# # Implementação de Round do Bernardo - estão faltando funções, então apenas usamos
# # para validar assinaturas das funções existentes (Comentado para evitar quebrar 
# # o código):
# ##################################################################################
# 
