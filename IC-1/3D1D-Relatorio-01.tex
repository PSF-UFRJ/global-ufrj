...
\begin{document}
	...
	\begin{table}
		\caption{Conjunto de variáveis necessárias para renderizar um único pixel na tela}\label{tab:newvar}
		\begin{tabular}{ll}
			\toprule[1.3pt]
			\thead{\textbf{Variável}} & \thead{\textbf{Descrição}} \\
			\midrule 
			\makecell{$n_Y$} & \makecell{Número de Pixels da Tela, número de pontos do reticulado de \\ representação da abertura de visão, em Y.} \\
			\makecell{$n_Z$} & \makecell{Número de Pixels da Tela, número de pontos do reticulado de \\ representação da abertura de visão, em Z.} \\
			\makecell{$(r_{ij}, g_{ij}, b_{ij})$} & \makecell{Vetor de cor original de $p$, valor intrínseco ao objeto representado \\ 
				pelo segmento visualizado.} \\
			\makecell{$(r_{fj}, g_{fj}, b_{fj})$} & \makecell{Vetor de cor renderizado (final) do subpixel $p_j$.} \\
			\makecell{$(r_f, g_f, b_f)$} & \makecell{Vetor de cor renderizado (final) de $p$.} \\
			\makecell{$I_L(D)$} & \makecell{Taxa de iluminação, dependente da distância $D$ do ponto iluminado em relação \\
				à fonte de luz.} \\
			\makecell{$I_d$} & \makecell{Taxa de iluminação difusa, constante para todos os pontos do ambiente.} \\
			\makecell{$P_i$} & \makecell{Pixel, de índice $i$, a ser renderizado} \\
			\makecell{$P_{ij}$} & \makecell{Componente do Pixel de índice $i$, com subíndice $j$, a ser renderizado} \\
			\makecell{$S_Y$} & \makecell{Largura total da tela, que representa a largura total de visão em Y}\\
			\makecell{$S_Z$} & \makecell{Largura total da abertura de visão em Z}\\
			\makecell{$S_{fY}$} & \makecell{Distância da borda da abertura de visão, em Y, até a posição do \\ foco de captura perante ela.} \\
			\makecell{$S_{fZ}$} & \makecell{Distância da borda da abertura de visão, em Z, até a posição do \\ foco de captura perante ela.} \\
			\makecell{$i_i$} & \makecell{Índice do Pixel, apenas para diferenciar da unidade imaginária  ($i$).} \\
			\makecell{$i_{ij}$} & \makecell{Índice do Componente do Pixel, subindexado em $j$.} \\
			\bottomrule[1.3pt]
		\end{tabular}
	\end{table}
